...
class TestRcloneHandler(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.rclone_handler = RcloneHandler(rclone_path="mock_rclone") # Use a mock path to instantiate RcloneHandler
    ...
    def test_list_remotes_failure(self):
        ...
        self.mock_subprocess_run.side_effect = subprocess.CalledProcessError(
            returncode=1, cmd=['mock_rclone', 'listremotes'], stderr="Error listing remotes"
        )
        with self.assertRaisesRegex(RuntimeError, "Rclone command failed: Error listing remotes"):
